...
# keepalive threads should remain below total worker threads
server.threads.keepalive=#{${server.threads.max} / 4}
```

## Properties Preprocessing

go-external-config provides the hook point necessary to modify values contained in the Environment. You can load property from external locations, for example AWS Systems Manager Parameter Store etc. See how this is implemented and works for Base64 decoding, random values generation and RSA decryption.
...
Example of initializing `http.Client` and `redis.Client` with default properties and overrides for the `component`:

```go
var httpClient *http.Client = env.ConfigurationProperties("component.httpClient", env.ConfigurationProperties("httpClient", &http.Client{
...
var redisClient *redis.Client = redis.NewClient(env.ConfigurationProperties("component.redis", env.ConfigurationProperties("redis", &redis.Options{})))
```
...
go-external-config provides a way to segregate parts of your application configuration and make it be available only in certain environments. Any `Bean` can be created with `Profile` to limit when it is loaded, as shown in the following example ([go-beans](https://github.com/go-beans/go) dependency required):

```go
ioc.Bean[NotificationSender]().Profile("kafka").Factory(NewKafkaNotificationSender).Register()
ioc.Bean[NotificationSender]().Profile("rabbitmq").Factory(NewRabbitMqNotificationSender).Register()
```
...
You can programmatically set active profiles by calling `env.SetActiveProfiles("...")` before your application runs. This can be useful for tests to mock `Bean`s or other scenarios.
