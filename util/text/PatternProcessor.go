...
type PatternProcessor struct {
	...
	resolve func(*regex.RegexpMatch) string
}
...
func PatternProcessorOf(pattern string) *PatternProcessor {
	return &PatternProcessor{
		...
		resolve: func(*regex.RegexpMatch) string { panic("Not implemented") }}
}
...
func (p *PatternProcessor) ProcessRecursive(str string, recursive bool) string {
	...
	for {
		...
		for _, match := range matched {
			...
			sb.WriteString(p.resolve(regex.RegexpMatchOf(p.regexp, resolved, match)))
			lastMatch = match
		}
...
func (p *PatternProcessor) OverrideResolve(f func(*regex.RegexpMatch,
	func(*regex.RegexpMatch) string) string) {

	super := p.resolve
	p.resolve = func(rm *regex.RegexpMatch) string { return f(rm, super) }
}
